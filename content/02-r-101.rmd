---
title: "R 101"
description: A simple introduction to working with data in R, including working with variables, functions and importing data.
section: Background
data:
    - codons.txt
---
...
**Side note for programmers**:  if you're coming to R with a background in other programming languages, you might have caught something in the last code block that freaked you out.  We had previously assigned the variable `c` to hold a number; yet we were still able to call the built-in `c` function from this scope.  In almost any other common scripting language this would not have worked.  Although base scoping rules in R are strictly lexical, there are a number of aggressive additional checks that are performed on lookup or evaluation failures.  For example, when an attempt is made to apply a non-function in an inner scope, the interpreter will ascend the scope chain looking for up values that *are* functions.  This is why we were able to bind `c` to a vector of numbers in our current scope and also apply the base R `c()` function.  Another example of this design philosophy at play in R is aggressive partial matching for named arguments in function calls.
...
As we saw in the previous section, vectors are the basic building blocks of all data in R and can hold numeric, string or boolean values.  Vectors can in turn be composed into more complicated data structures including matrices, arrays, data frame's and lists.
...
Unlike arrays and matrices, lists are collections of vectors where the individual vectors can hold different types of data:
...
To follow along with this example, you can download the genetic code table and save it in a subfolder named `data/` in your current working directory:  [codons.txt](data/codons.txt).

This table has two columns:  "codon" and "aminoAcid."  To load the table into a variable:
...
In the samples above we've used some of the stock functions provided by the base R environment, like `c` and `read.table`.  So far, I've been giving you the syntax to copy when calling these functions but it's important for you to see how you can figure out how to use functions from their documentation.  

## Getting help

Use the help function to pull up the documentation page for the `read.table` function:
...
Manual pages for all of the functions in base R and packages are built into the language so you'll find that these pages always follow a standard formatting.

If you are working in [RStudio](http://www.rstudio.com/) you can also search for help using the search box on the Help tab (lower right panel by default).  Finally, can a get the help page for a function by putting your cursor over its name in the editor or console and hitting F1.

The "Usage" and "Arguments" sections tell us what values this function requires or can accept.  Values passed into functions are called arguments.  For `read.table` you'll see that there's an extensive list of possible arguments.  

You'll notice that the arguments are listed in a specific order and they each have names.  In R, you can specify arguments either by their position or by their names, or both!

##

For example these lines are all equivalent calls to the `read.table` function:
...
Function arguments can the required or optional in R.  In the "Usage" section of the help page, you'll notice that `file` is listed on its own, but `header` is written as `header = FALSE`.  This means that the `read.table` function provides a default value for the `header` argument: if you don't specify a `header` argument, `read.table` will assume you want it to be `FALSE` (no header).  In general, you'll find that many stock R functions take one or two required "data" arguments followed by a long list of optional arguments with reasonable defaults.  Some functions, like this one, will have long "Details" sections that offer more information about how the arguments interact or advanced usages of the function.

The "Value" section of the documentation tells you what kind(s) of values the function returns.  In R all function evaluations produce a value (there are no `voids`).

Because R was built to be used for research, most function documentation pages will also have a "References" section to tell you what you should cite if you use that function to do data analysis for a publication.  Last, but definitely not least, nearly all R help pages will end with a set of example usage scenarios.

##

Let's look at the help page for the `c` function to see an example of a special type of argument:

```{r eval=FALSE}
?c
```

Here we see that the `c` function takes a special argument named `...`.  This ellipsis means a "variable number" of arguments, used by position.  This is why `c` is able to understand what we mean by all of the following:
...
c(1, 2)
c(1, 2, c(3, 4, 5))
c(1, 2, c(3, 4, 5), recursive = TRUE)
```

## Writing functions

You can also write your own functions.  If you find yourself repeating the same set of steps many times in a data analysis project with only minor changes, it's generally a good idea to think about [abstracting](http://en.wikipedia.org/wiki/Abstraction_(computer_science)) those steps into a function.

The syntax for defining a function follows what you've just seen in the help documentation:
...
Here you can see that we "assigned" the function to a variable named `f`.  Functions are "[first class](http://en.wikipedia.org/wiki/First-class_function)" citizens in R meaning that they can be assigned to variables and even passed to other functions just like any other kind of data.  To call our function we apply it to two values:
...
You can also specify default values for your functions:
...
You can see above that when our function is run the value produced is `a + b`.  Functions in R evaluate to the value of the last expression that they execute.  But they can do other stuff beforehand:

```{r}
f <- function(a, b) {
    print("I'm in your function adding stuff.")
    a + b
}
f(1, 2)
```
...
If you're coming from other programming languages (not in the functional programming lineage) it might seem strange to not specify a "return" value.  If that makes you uncomfortable (or if you want to explicitly return early) R provides a `return` function:
...
There are a ton of functions available to you in the base R environment, but MANY more can be added by importing packages.  Packages are plugins to the base R system designed to add features suited to particular tasks.  The package ecosystem is one of the greatest strengths of R.  

There are two majors repositories for R packages:  [CRAN](http://cran.r-project.org/index.html) and [BioConductor](http://www.bioconductor.org/).  CRAN is for general packages, Bioconductor for bioinformatics and computational biology.

The best way to get start with CRAN is [Task Views](http://cran.r-project.org/web/views/).  I've setup RNA with a bunch of packages I think are useful, but to install more for your account use `install.packages`:

```{r eval=FALSE}
install.packages("ggplot2")
```

Once packages are installed, you can load them into your current session using `library`:

```{r eval=FALSE}
library(ggplot2)
```

To get help for a package, use `?`:

```{r eval=FALSE}
?ggplot2
```

## Advanced Notes on Functions

This section is included for folks coming to R with lots of experience in other scripting languages; if that's not you, you might want to skip this!  You have been warned.  

The first important thing to understand about R functions is that there is only one type: anonymous closures.  R functions only have names in the sense that they can be assigned to named variables; there's no inherent concept of a named function in R (in contrast to many other high-level scripting languages).  A closure is a function that "closes over" variables that are in-scope when it is defined.  Take this example:
...
In this code, `foo` is a function that returns another function.  So, `foo(1)` returns a version of our inner function where `x` is bound to `1`.  On the line `foo(1)(2)` we apply the returned inner function to the argument `2`.  You'll find a lot of functions in the R standard library that make liberal use of this language feature to allow you to pass in functions that implement alternative algorithms.

The second important thing to understand about R functions that might surprise you is that default arguments are evaluated **lazily** (like everything else in R) *in the function scope*.  You might find this last bit a little mind blowing.  Here's an example:
...
You'll also see liberal use of this technique in the functions in the R standard library.  Did I mention yet that there are no statements in R, only expressions?  Take this example using an `if` expression:

```{r}
f <- function(a, b = if (a < 10) { 1 } else { 10 }) {
    a + b
}
f(1)
f(11)
